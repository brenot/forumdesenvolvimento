...
class plgContentAddToAny extends JPlugin
{
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		$css = '.addtoany_larger_textarea { width: 400px; max-width: 100%; } ';
		$css .= '.addtoany_smaller_text { width: 26px; }';
	...
	private function createAddToAny($article)
	{
		...
		(
			...
			$app->input->get('id') == $menu_default->query['id'] &&
		...
		$doc->addScript('//static.addtoany.com/menu/page.js', 'text/javascript', false, true);
		...
		elseif ($this->params->get('categories_excluded_2', 0) != 0)
		{
			...
			if (in_array($article->catid, $this->params->get('categories_excluded_2', array())))
			{
				// Disable sharing
				$output = '';
			}
		...
		elseif ($this->params->get('categories_excluded', 0) != 0)
		{
			...
			// If current (sub)category is excluded
			if (in_array($article->catid, $categories_excluded))
			{
				// Disable sharing
				$output = '';
			}
	...
	private function createServiceButtons()
	{
		$universal_after = '';
		$universal_before = '';
		$universal_html = '<a class="a2a_dd" href="https://www.addtoany.com/share_save">' . $this->params->get('universal_button_innerhtml', '') . '</a>' . PHP_EOL;
	...
	private function createAdditionalJSOnce($doc)
	{
		...
		{
			$doc->addScriptDeclaration('var a2a_config=a2a_config||{};' . $additional_js);
			$created = true;
		}
	}
